...
function cardHTML(i,owner){
  ...
  if(!taken){
    ...
    actions+=`<button class="btn btn-wa" data-act="wa" data-id="${i.id}">
      ...
      ${waLabel}</button>`;
    actions+=`<button class="btn btn-ghost btn-sm btn-block" style="margin-top:8px" data-act="gone" data-id="${i.id}" data-need="${need?1:0}">${need?'Ya no se necesita':'Ya no está disponible'}</button>`;
